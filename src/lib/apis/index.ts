...
export const getModels = async (
	...
	let models = res?.data ?? [];
	// NOTE: The legacy "directConnections" feature fetched models directly from the browser (CORS + OpenAI-compatible).
	// We intentionally keep the function signature for backwards compatibility but no longer merge local models here.
	void connections;

	return applyModelIcons(models) ?? models;
};
