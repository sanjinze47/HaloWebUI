...
// highlight.js loaded dynamically in copyToClipboard() to reduce initial bundle
...
/**
 * Detect animated images (GIF with multiple frames, animated WebP).
 * Returns true if the data URL represents an animated image that should
 * be preserved as-is (skip JPEG compression to keep the animation).
 */
export const isAnimatedImage = (file: File): boolean => {
	if (file.type === 'image/gif') return true;
	// Animated WebP detection by extension (full frame detection requires parsing)
	if (file.type === 'image/webp') {
		// WebP could be animated; preserve to be safe
		return true;
	}
	return false;
};
		...
		img.onload = () => {
			...
			if (maxWidth && maxHeight) {
				...
				if (width <= maxWidth && height <= maxHeight) {
					// Image is smaller than max, but still compress quality
					canvas.width = width;
			...
			// else: No dimensions specified - keep original size but compress quality
			...
			// Use JPEG with quality=0.85 for smaller file sizes (typically 5-10x smaller than PNG)
			const compressedUrl = canvas.toDataURL('image/jpeg', 0.85);
			resolve(compressedUrl);
		};
...
export function avatarHashCode(s: string): number {
	let h = 0;
...
export const generateInitialsImage = (name) => {
	...
	ctx.fillStyle = AVATAR_PALETTE[avatarHashCode(name || '') % AVATAR_PALETTE.length];
...
/** Strip raw thinking/reasoning blocks (e.g. <think>...</think>, <reasoning>...</reasoning>) */
export const stripThinkingBlocks = (content: string): string => {
	return content.replace(/<(think|thinking|reasoning)>[\s\S]*?<\/(think|thinking|reasoning)>/g, '');
};
