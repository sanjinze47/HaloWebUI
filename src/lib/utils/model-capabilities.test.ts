...
describe('image generation model capability inference', () => {
	...
	it('keeps video generation independent from image generation', () => {
		expect(isDedicatedVideoGenerationModel('grok-imagine-video')).toBe(true);
		expect(inferModelCapabilities('grok-imagine-video').videoGen).toBe(true);
		expect(inferModelCapabilities('grok-imagine-image').videoGen).toBe(false);
	});
